...
(async()=>{
 ...
 const about=await browser.newPage({viewport:{width:1440,height:1000}});await about.goto('http://127.0.0.1:4173/about',{waitUntil:'networkidle'});await about.waitForFunction(()=>window.__aboutDebug?.state.avatarLoaded);await about.waitForTimeout(1400);
 const baseline=await about.evaluate(()=>({state:window.__aboutDebug.state,hasMarble:!!window.__aboutDebug.marbleReveal,originalLink:!!document.querySelector('[data-version="original"]')}));assert.equal(baseline.state.variant,'marble');assert.equal(baseline.hasMarble,true);assert.equal(baseline.originalLink,false);
