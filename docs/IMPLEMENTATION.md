...
- Avatar: user's textured static GLB, normalized to a fixed visual size; smooth pointer/orientation rotations, raycast-local blue shader, global click spring response. There are no facial animation clips in the supplied model.
...
User avatar source stays unchanged in its original directory. A reduced web copy is saved under `dist/assets/avatar.glb`. The previous procedural placeholder is an original geometric character and is used only as a load-error fallback. No reference-site model, proprietary font or identity assets are included.
